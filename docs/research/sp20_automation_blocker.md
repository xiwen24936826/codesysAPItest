...
- 在 2026-04-22，已对用户目录下的 `APInstaller` 配置执行保守重置：
  - 原目录已备份
  - 新目录已重建为空目录
- 该操作用于排查 `APInstaller.CLI.exe --discoverAddOnUpdates` 引发的弹窗问题
- 该问题仍应视为环境阻塞项，除非后续多轮真实联调都不再出现该弹窗
...
- POU 主线的“手工项目 + 临时副本 + 根容器回退”真实链路已经恢复并成功通过一次
- 基于 `ServerRuntime` 的第一条 MCP server 端到端真实链路也已成功通过一次
- 但这并不自动意味着 `APInstaller.CLI.exe` 弹窗问题已经彻底根除
- 后续仍需继续观察多次真实自动化调用时是否还会触发安装器异常
...
- POU 第一阶段主线已经在真实项目上成功跑通一次
- 说明服务层、桥接层和手工项目驱动联调路径已经具备继续推进的基础

所以现阶段最合理的决策是：

- 保留已通过的 POU 主线路径
- 继续记录和排查安装器弹窗问题
- 在不扩大真实调用频率的前提下，谨慎推进后续真实联调
